...
app.layout = dbc.Container(
    [
        ...
        dbc.Card(
            [
                dbc.CardHeader(html.H5("Powertek MC361 - Temperature simulation")),
                dbc.Form(
                    [
                        dbc.CardGroup(
                            [
...
def disable_start(
    ...
    if (
        ...
        or step_interval is None
        or test_cycles is None
    ):
        return True
...
def run_test(
    ...
    if n_clicks and INCLUDE_TEST:
        while CYCLES >= 1:
            ...
            if INTERFACE == "serial":
                print("serial")
                mc361_serial.run_temp_test(
                    SER_PORT, SER_BAUD, MIN_TEMP, MAX_TEMP, TIME_INTV, STEP_INTV
                )
            elif INTERFACE == "ethernet":
                print("ethernet")
                mc361_eth.run_temp_test(
                    IP, 23, MIN_TEMP, MAX_TEMP, TIME_INTV, STEP_INTV
                )
            CYCLES -= 1
        CYCLES = 1
        STATUS = "Waiting..."
